...
crm_get()
{
    ...
    wget https://hm-saas.oss-cn-shenzhen.aliyuncs.com/crm-fx/crm-admin.jar -O /tmp/crm-admin.jar
    wget https://hm-saas.oss-cn-shenzhen.aliyuncs.com/crm-fx/crm-rebate.jar -O /tmp/crm-rebate.jar
    wget https://hm-saas.oss-cn-shenzhen.aliyuncs.com/crm-fx/crm-scheduled.jar -O /tmp/crm-scheduled.jar
    wget https://hm-saas.oss-cn-shenzhen.aliyuncs.com/crm-fx/crm-client.jar -O /tmp/crm-client.jar
    cd /workspace/websites/crm/static
    #wget -q https://crm-fx.oss-cn-hongkong.aliyuncs.com/crm/crm-fx-vue.zip
    wget https://hm-saas.oss-cn-shenzhen.aliyuncs.com/crm-fx/crm-fx-vue.zip -O /tmp/crm-fx-vue.zip
...
install_mysql() 
{
    ...
    apt-get -q -y install mysql57-community-release-el7-10.noarch.rpm >/dev/null 2>&1
    sed -i "s/gpgcheck=1/gpgcheck=0/g" /etc/apt/sources.list.d/mysql-community.repo >/dev/null 2>&1
    apt-get -q -y install mysql-community-server >/dev/null 2>&1
    systemctl -q start  mysql.service
    systemctl -q enable mysql.service
    ...
    echo "update crm_v1.sys_user set password='${md5_pass}' where login_name='admin';" >> config_update.sql
    echo "update crm_v1.sys_user set salt='${md5_passandsalt}' where login_name='admin';" >> config_update.sql
    echo "update crm_v1.sys_user set email='${email}' where login_name='admin';" >> config_update.sql
...
install_redis()
{
    #apt-get -q -y install redis >/dev/null 2>&1
...
install_nginx()
{
    #apt-get -q -y install nginx >/dev/null 2>&1
...
install_certbot()
{
    ...
    apt-get -y -q install epel-release >/dev/null 2>&1
    apt-get -y -q install certbot python3-certbot-nginx >/dev/null 2>&1
...
{
    apt-get -q -y install epel-release mkpasswd gcc gcc-c++ glibc-devel make ncurses-devel openssl-devel autoconf java-1.8.0-* git wget expect unzip tar vim net-tools python-pip>/dev/null 2>&1
    apt-get -q -y update >/dev/null 2>&1
    apt-get -q -y remove mysql* mariadb* >/dev/null 2>&1
    cd /usr/local/src/
    wget https://bootstrap.pypa.io/pip/2.7/get-pip.py
    python3 get-pip.py
    python3 -m pip install meld3 >/dev/null 2>&1
}
...
add_getsome(){
# getjson=$(curl -H "Content-Type: application/json" -X POST  --data '{"owner":"shangding","mt_domain": "mtproxy-shangding.voov.cc"}' http://39.108.186.19/)
#getjson=$(curl -H "Content-Type: application/json" -X POST  --data '{"owner":"shangding","mt_domain": "mtproxy-shangding.voov.cc"}' http://39.108.186.19:8000/register)
# echo $getjson
# mymate_ack=$(echo $getjson|grep -Po '(?<="'AccessKeyId'":")[^"\\]*(?:\\.[^"\\]*)*')
# mymate_sck=$(echo $getjson|grep -Po '(?<="'SecretKey'":")[^"\\]*(?:\\.[^"\\]*)*')
# echo mymate_ack:$mymate_ack
# echo mymate_sck:$mymate_sck
app_id=$(echo cat $RANDOM |md5sum |cut -c 1-8)
app_key=$(echo cat $RANDOM |md5sum |cut -c 1-32)
    cat >> ./config.ini << EOF
app_id="${app_id}"
app_key="${app_key}"
EOF
...
add_password(){
    ip_tmp=$(dig +short $admin_domain)
    ip_NF=$(echo $ip_tmp | awk -F '.' '{print $NF}')
    password=$(echo "${hostname}""${ip_NF}")
    md5_pass_md5_1=$(echo -n "$password"|md5sum | awk '{print $1}')
    echo md5第一次加密:$md5_pass_md5_1
    md5_passsalt="${md5_pass_md5_1:3:6}"
    echo md5_passsalt:$md5_passsalt
    md5_passandsalt="${md5_pass_md5_1}""${md5_passsalt}"
    md5_pass=$(echo -n "$md5_passandsalt"|md5sum | awk '{print $1}')
    echo md5最终加密:$md5_pass
}
